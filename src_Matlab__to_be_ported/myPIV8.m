  ...
  for iIW_size = 1:nIW_SIZES
    ...
    for iIW = 1:IW_grid_A.nIWs
      ...
      if iIW_size == 1
        ...
        shift_x = VMs{iIW_size - 1}.dx(iIW_parent);   % [px]
        shift_y = VMs{iIW_size - 1}.dy(iIW_parent);   % [px]
        if isnan(shift_x); shift_x = 0; end
        if isnan(shift_y); shift_y = 0; end
      
        % Calculate new center and range of the IW in frame B using shift
      ...
      if isempty(img_IW_A(:)) || ...
       ...
       %           double(img_IW_A));   % Slow & accurate
        C = xcorr2(single(img_IW_B), ...
                   single(img_IW_A));   % Fast & slightly less accurate
        C = C/max(C(:));                % Normalize
      end
      ...
      if fDEBUG && iIW == iIW_debug && numel(C) > 1
        ...
        % Show quiver
        quiver(h1_ax, VM.x(iIW), VM.y(iIW), VM.dx(iW), VM.dy(iW), ...
    ...
    VM.magn  = sqrt(VM.dx.^2 + VM.dy.^2);
    VM.angle = atan2(VM.dy, VM.dx);           % [rad]
    VM.angle(VM.magn == 0) = nan;             % make undefined angle nan
    ...
    if 1
      VM_filt = replace_bad_vectors_by_nn(VM_filt, iIW_size ~= nIW_SIZES);
    end
    
    if 1
      [iBadAngle, iBadMagn] = detect_bad_vectors(VM_filt);
...
if 1
  % Show original full figure A with quiver map on top
  h6 = figure(6); clf
