...
for iFile = 1:1
  ...
  if fSHOW_XCOR 
    h2 = figure(2); clf
    set(h2, 'Position', [650, 685, 640, 640])
  end
...
%                '-y', 'LineWidth', 2)
        ...
        if fSHOW_XCOR && numel(C) > 1
          figure(h2); cla
          imshow(flipud(C)*255, jet(256), 'InitialMagnification', 300)
          set(gca, 'YDir', 'normal')
...
vector_map_filtered(:, :, 1) = vmf_x;
vector_map_filtered(:, :, 2) = vmf_y;
vector_map_filtered(:, :, 3) = vmf_dx;
vector_map_filtered(:, :, 4) = vmf_dy;
