...
for iFile = 1:1
  ...
  if fSHOW_XCOR 
    h2 = figure(2); clf
    set(h2, 'Position', [850, 1085, 256, 256])
  end
...
h1 = figure(1); clf
set(h1, 'Position', [60, 60, 1024, 800])
%imshow(imadjust(A), 'InitialMagnification', 80, 'Colormap', bone(256));
imagesc(imadjust(A)); axis equal
colormap(bone(256))
hold on
tmp = get(h1, 'Position');
set(h1, 'Position', [2626, -240, tmp(3), tmp(4)])
...
if 0
  quiver(vector_map(:, :, 1), ...
         vector_map(:, :, 2), ...
         vector_map(:, :, 3) * vector_magnificier, ...
         vector_map(:, :, 4) * vector_magnificier, ...
         vector_scale, 'r', 'LineWidth', 2)
end
xlim([1 img_w])
ylim([1 img_h])

vel_mag = sqrt(vector_map(:, :, 3).^2 + vector_map(:, :, 4).^2);
...
vector_map_filtered = reshape(vector_map_filtered, numel(vector_map_filtered), 1);
vector_map_filtered(iMatch) = nan;
vector_map_filtered = reshape(vector_map_filtered, size(vector_map));
%vector_map_filtered(iMatch_I, iMatch_J, :) = nan;
...
vel_mag_img = interp2(vector_map(:, :, 1)', ...
                      vector_map(:, :, 2)', ...
                      vel_mag', ...
                      px_x, px_y, ...
                      'nearest');
...
%image(vel_mag_img)
img=imfuse(vel_mag_img, ind2rgb(imadjust(A), bone(255)), 'blend');
imshow(img)
axis equal
xlim([1 img_w])
ylim([1 img_h])
